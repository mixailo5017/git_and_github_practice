...
jQuery(document).ready(function(){
	...
	jQuery('.geocode_member').click(function(e){
		var $a = jQuery(this);


		jConfirm('HTML here!','title',callback_func);
		return false;
	});
	...
	jQuery('.changetheme a').click(function(){
		...
		if(jQuery('#addonstyle').length == 0) {
			if(c != 'default') {
				jQuery('head').append('<link id="addonstyle" rel="stylesheet" href="/themes/css/style.'+c+'.css" type="text/css" />');
				jQuery.cookie("addonstyle", c, { path: '/' });
			}
		} else {
			if(c != 'default') {
				jQuery('#addonstyle').attr('href','/themes/css/style.'+c+'.css');
				jQuery.cookie("addonstyle", c, { path: '/' });
			} else {
