		...
		if (! $CI->upload->do_upload($filename)) {
            ...
            if (isset($image_data['image_width']) &&
                ...
                if ($image_data['image_width'] * $image_data['image_height'] > MAX_IMAGE_DIMENSIONS) {
                    ...
                    if (! $error) $error = 'Image dimensions are too big.';
	...
	function check_is_topexpert($uid)
	{
		...
		if($qryusersector->num_rows() > 0)
		{
			...
			foreach($qryusersector->result_array() as $row) {
				if($row["sector"] != "") {
					if($sector == "") { $sector = "'".$row["sector"]."'"; } else { $sector .= ",'".$row["sector"]."'"; }
				}
				if($row["subsector"] != "") {
					if($subsector == "") { $subsector = "'".$row["subsector"]."'"; } else { $subsector .= ",'".$row["subsector"]."'"; }
				}
...
//		if (! sess_var('logged_in')) {
        if (! $CI->auth->check()) {
            ...
            if (empty($intended) || $intended == '/') redirect(index_page(), 'refresh');

			// Check if there are any query parameters
            $query_string = $_SERVER['QUERY_STRING'];
            if (! empty($query_string)) $intended .= '?' . $query_string;
...
if (! function_exists('email')) {
    ...
    {
        if (empty($recipients) || empty($sender)) return false;
