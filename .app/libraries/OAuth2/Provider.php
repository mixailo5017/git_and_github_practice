...
abstract class OAuth2_Provider
{
	...
	public function __construct(array $options = array())
	{
		...
		$this->redirect_uri = isset($options['redirect_uri']) ?
			$options['redirect_uri'] : site_url(get_instance()->uri->uri_string());
	}
	 ...
	 * @param OAuth2_Token_Access $token
	 * @param $fields
	 * @return array extended user info
	 */
	abstract public function get_user_info2(OAuth2_Token_Access $token, $fields);

	/**
	 * @param OAuth2_Token_Access $token
	 * @param $company_id
	 * @param $fields
	 * @return array user's company info
	 */
	abstract public function get_company_info(OAuth2_Token_Access $token, $company_id, $fields);
	...
	*/	
	public function authorize($options = array())
	{
		...
		get_instance()->session->set_userdata('oauth2.state', $state);
